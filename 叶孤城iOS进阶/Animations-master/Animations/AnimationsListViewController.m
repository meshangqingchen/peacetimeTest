...
- (void)setup {
    ...
    [self rootViewControllerSetup];  //navation 的delegate 手的deleagte.
    
    [self configureDataSource];      //创建item对象 存在一个数组里.
    
    [self configureTableView];       //创建tableView.
...
- (void)rootViewControllerSetup {
    ...
    // [IMPORTANT] Set the RootViewController's push delegate. //手势的delegate
    [self useInteractivePopGestureRecognizer];
}

#pragma mark - Push or Pop event. //转场动画
